...
def merge(source_path: str, dest_path: str):
  ...
  while fnames:
    ...
    while fnames and totaldur + fdur <= 600.0:
      print(fnames.pop(0)) # pop the current last item in merge (the one that corresponds with nextdur)
    ...
    # so concat
    if last == absfirst:  # if no files concat with first file, then just copy first
      shutil.copy(absfirst, dest_path)
      continue
